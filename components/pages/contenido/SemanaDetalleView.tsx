...
/**
 * Detalle de una semana del plan: episodio madre, activos derivados, notas,
 * clase y métricas, en seis pestañas.
 *
 * Cuelga del Calendario Maestro (`/contenido/calendario-maestro/semana-N`),
 * que es de donde viene el usuario al pulsar "Ver detalles" en una semana.
 */
export function SemanaDetalleView({ week }: { week?: number }) {
  ...
  const { metaFields, updateMetaFields, blocks, addBlock, updateBlock, removeBlock } = usePageConfig(
    `/contenido/calendario-maestro/semana-${week ?? "actual"}`,
    [
   ...
   * detalle cargado y, en último caso, la primera en producción.
   */
  const episode = useMemo(() => {
    if (week != null) {
      const match = episodes.items.find((e) => e.week === week);
  ...
  if (loading) {
    return (
      <PageShell title={week ? `Semana ${week}` : "Detalle de la semana"} icon="CalendarRange" starrable={false}>
  ...
  if (!episode) {
    return (
      <PageShell
        title={week ? `Semana ${week}` : "Detalle de la semana"}
        description="El detalle completo de la semana: episodio madre, activos derivados, notas, clase y métricas."
        icon="CalendarRange"
        starrable={false}
      >
        <div className="surface-card">
          <EmptyState
            icon="CalendarRange"
            title="Esta semana todavía no tiene episodio"
